...
For the Sunday/Tuesday/Thursday 6 PM cadence, import `backend/docs/workflows/n8n/sunday-wednesday-lead-cadence.json` (filename retained for stability; the workflow now runs Sunday, Tuesday, and Thursday at 18:00 `America/Vancouver`), keep it inactive until the environment variables above resolve, then run the health and normal `/run` nodes before activation.
