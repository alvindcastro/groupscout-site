...
Current separate UI repo note: `/mnt/c/Users/alvin/WebstormProjects/groupscout-ui` has a D4 lightweight Node production server that serves `web/dist` and proxies `/api/*` server-side to `http://groupscout:8080`, plus a `smoke-ui-e2e` Compose profile. Source-snapshot reconciliation on 2026-05-25 found the inspected UI checkout missing the documented admin/session/detail-client/hardening artifacts and the inspected backend checkout missing the repeatable smoke target. Active follow-ups are tracked in Beads: `groupscout-site-0m0` for UI checkout reconciliation, `groupscout-site-crz` for backend EvalOps/smoke artifact reconciliation, `groupscout-site-kb4` for the real-browser Phase 15 harness, `groupscout-site-29q` for generated API client/types, `groupscout-site-4cv` for sanitized raw preview, `groupscout-site-3gq` for shared alert state, and `groupscout-site-2h1` for broader smell refactors.
