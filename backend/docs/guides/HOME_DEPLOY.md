...
For the daily (except Saturday) cadence, import `backend/docs/workflows/n8n/sunday-wednesday-lead-cadence.json` (filename retained for stability; the workflow now runs daily except Saturday), keep it inactive until the environment variables above resolve, then run the health and normal `/run` nodes before activation.
