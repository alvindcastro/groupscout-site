...
Current separate UI repo note: `/mnt/c/Users/alvin/WebstormProjects/groupscout-ui` has a D4 lightweight Node production server that serves `web/dist`, applies UI session authorization before proxying `/api/*` server-side to `http://groupscout:8080`, and includes a `smoke-ui-e2e` Compose profile. Source-snapshot reconciliation on 2026-06-14 restored the current checkout's detail client, production session gating, and deterministic Phase 15 artifacts; the inspected backend checkout still lacks the repeatable smoke target. Active follow-ups are tracked in Beads: `groupscout-site-crz` for backend EvalOps/smoke artifact reconciliation, `groupscout-site-eqm` for backend UI/auth routes, `groupscout-site-1x9` for frontend `/admin/login` after backend auth lands, `groupscout-site-kb4` for the real-browser Phase 15 harness, `groupscout-site-29q` for generated API client/types, `groupscout-site-4cv` for sanitized raw preview, `groupscout-site-3gq` for shared alert state, and `groupscout-site-2h1` for broader smell refactors.
