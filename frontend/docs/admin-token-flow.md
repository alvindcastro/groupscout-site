...
Status reconciliation, 2026-06-14: this is the planned admin-token runbook for the auth branch/reconciliation work. The inspected backend source snapshot does not expose `/api/auth/*`, and the inspected UI checkout does not expose `/admin/login` or the documented auth client wiring. Backend implementation is tracked by `groupscout-site-eqm`; frontend login restoration is tracked by `groupscout-site-1x9`. In the current checkout, a missing `/admin/login` route is expected and should not be treated as stale Docker assets.
