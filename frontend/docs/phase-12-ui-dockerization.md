...
- Backend-dependent smoke now has a repeatable backend-owned gate: run `make smoke-ui-docker-e2e` from `/mnt/c/Users/alvin/GolandProjects/groupscout`. The gate uses the UI repo's `smoke-ui-e2e` Compose profile and checks production `/healthz`, `/`, `/assets/app.js`, proxied `/api/system`, and a bad-proxy `502`.
